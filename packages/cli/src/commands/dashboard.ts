...
function renderProviderScanPanel(): string {
  return [
    ...
    '</select>',
    '<button class="btn" onclick="scanProviderTopics()">Scan Topics</button>',
...
export function renderLocalDashboardHtml(config: ProjectConfig, memories: Memory[]): string {
  ...
  return [
    ...
    '    setSetupDetail(status.connectorReachable ? (status.scanReady ? "Connector launch and provider scan are active." : "Connector launch active. Provider scan is not exposed by the connector.") : "Fallback path active: opens provider and copies prompt. Provider scan needs connector.");',
    '    hideSetupPanel();',
    '  } else {',
    '    setupState.completed = false;',
    '    showSetupPanel();',
    '    setSetupSubtitle("One-time setup is still needed. Run Test Launch once to finish.");',
    '    setSetupDetail(status.connectorReachable ? (status.scanReady ? "Connector detected for launch and scan. Run Test Launch to confirm daily flow." : "Connector detected for launch. Scan support is not exposed.") : "Connector not detected. Daily fallback still works after Test Launch.");',
    '  }',
    '  saveSetupState();',
    ...
    '    "Instruction: Continue from this context and confirm assumptions before proceeding.",',
    '  ].filter(Boolean).join("\\n");',
    ...
    '    return;',
    '  }',
    '  setScanStatus("Scanned " + scannedTopics.length + " topic(s) from " + providerLabel(sourceProvider) + ".");',
    '}',
    ...
    '  runSetupFlow().then(() => {',
    '    if (setupState.connectorReachable) scanProviderTopics();',
    '    else {',
    '      renderScannedTopics();',
    '      setScanStatus("No provider connected. Enable a connector, then scan topics.");',
    '    }',
    '  });',
    '  loadImported();',
    '  renderImported();',
