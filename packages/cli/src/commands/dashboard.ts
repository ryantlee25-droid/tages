...
function renderLocalDashboardHtml(config: ProjectConfig, memories: Memory[]): string {
  ...
  return [
    ...
    '<div class="focus-note">Flow: choose target provider -> click a pack tile -> launch with copied context. Packs stay provider-agnostic.</div>',
    '<div class="grid">',
    ...
    '  el.textContent = target.enabled ? "Ready: direct launch adapter enabled." : "Adapter scaffolded: opens provider and copies prompt.";',
    '}',
    ...
    '}',
    'function launchPack(pack, explicitTargetProvider) {',
    ...
    '  const enriched = packs.map((pack) => ({ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, ...pack, updatedAt: new Date().toISOString() }));',
    ...
    '  if (targetProviderEl) targetProviderEl.addEventListener("change", updateTargetProviderStatus);',
