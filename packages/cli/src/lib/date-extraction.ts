/**
 * CLI-local rule-based date extraction for 3-date temporal anchoring
 * (migration 0060).
 *
 * Mirrors packages/server/src/temporal/date-extraction.ts's absolute +
 * relative date parsing. Deliberately NOT imported from @tages/server: a
 * runtime dependency on the server package would break `npm install -g
 * @tages/cli` standalone installs (same reasoning as lib/embedding.ts and
 * lib/chunking.ts). Keep this file in sync with the server's copy by hand
 * if either the regex patterns or resolution logic changes.
 *
