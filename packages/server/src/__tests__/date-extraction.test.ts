...
import { extractDates, extractDatesFromMemory } from '../temporal/date-extraction'
...
describe('extractDates — "N months ago" month-end overflow (correctness)', () => {
  it('does not overflow Feb when subtracting a month from the 31st', () => {
    // 2026-03-31 minus 1 month must clamp to 2026-02-28, NOT roll forward to
    // "Feb 31" -> 2026-03-03 (the setUTCMonth bug).
  ...
  it('clamps to the last day of a leap-February when subtracting 12 months from a 29th', () => {
    // 2025-02-28 is the anchor day-of-month source: from 2027-02-28 back 12
    // months lands in 2026-02, which has 28 days — day 28 is valid, unchanged.
    const anchor = new Date('2026-05-31T00:00:00.000Z')
    const result = extractDates('happened 3 months ago', anchor)
    // May 31 back 3 months = Feb 28 2026 (Feb has 28 days, clamps from 31).
...
describe('extractDatesFromMemory — prefers the key over incidental value dates', () => {
  ...
  it('falls back to the value date when the key carries none', () => {
    const result = extractDatesFromMemory(
      'auth rollout',
      'we shipped this on 2026-07-01',
      ANCHOR2,
    )
    expect(result.referencedDate).toBe(new Date('2026-07-01T00:00:00.000Z').toISOString())
  })

  it('returns {} for undefined referenced/relative when neither key nor value has a date', () => {
